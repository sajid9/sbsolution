...
class Ledger_supplier extends Controller
{
    ...
    public function get_customer(Request $request){
    	$customers = customers::select('id','customer_name as text')->where('customer_name','like','%'.$request->term.'%')->get();
    	return json_encode($customers);
    }
    public function get_receipt(Request $request){
        $receipt = receipt::select('id','receipt_no as text')->where('receipt_no','like','%'.$request->term.'%')->get();
